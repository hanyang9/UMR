import { chamferL2CPU, pointRepulsionLossCPU } from "./umr-exact-training-neighbors-cpu-v4.js";

export { TorchAdamW, cosineAnnealingLearningRate,
  exactCorrespondenceTrainingContract } from "./umr-exact-training-kernels-v1.js";
export const chamferL2 = chamferL2CPU;
export const pointRepulsionLoss = pointRepulsionLossCPU;
export function residualRegularizationLoss(tf, residual) { return residual.square().mean(); }
    ...
    for (let batch = 0; batch < batches; batch += 1) {
      ...
      for (let edge = 0; edge < edgeCount; edge += 1) {
        ...
        for (let axis = 0; axis < 3; axis += 1) {
          const delta = Number(values[source + axis]) - Number(values[target + axis]);
          total += delta * delta;
          gradient[source + axis] += scale * delta;
          gradient[target + axis] -= scale * delta;
        }
      }
    }
    return {
...
export function correspondenceLoss(tf, options) {
  const { reconstruction, target, residual, edgeIndex,
    chamferWeight = 1, repulsionWeight = 0.002, repulsionK = 8,
    repulsionRadius = 0.035, residualWeight = 0, edgeWeight = 0.4 } = options;
  ...
  const total = chamfer.mul(chamferWeight).add(repulsion.mul(repulsionWeight))
    .add(residualRegularization.mul(residualWeight)).add(edgeSmoothness.mul(edgeWeight));
