...
export async function retargetMotionBrowserExact(
  ...
  onProgress(0, 0, frames, "Initializing the Clarabel QP solver…");
  await nextPaint();
  await initializeClarabelQP();
  throwIfAborted(signal);
  onProgress(0.01, 0, frames, "Clarabel ready · preparing robot-object collision constraints…");
  await nextPaint();
  const { model, data, module } = viewer;
  const nv = Number(model.nv);
  ...
  onProgress(
    ...
    frames,
    collisionCache
      ? "Robot-object collision model ready · starting frame optimization…"
      : "Retargeting constraints ready · starting frame optimization…"
  );
  await nextPaint();
  ...
  const solveOrder = async (order) => {
    ...
    for (let cursor = 0; cursor < order.length; cursor += 1) {
      ...
      progressDone += 1;
      const solveFraction = progressDone / progressTotal;
      onProgress(0.03 + 0.97 * solveFraction, frame + 1, frames);
      await nextPaint();
      throwIfAborted(signal);
    }
