// Exact exhaustive Chamfer and repulsion values/first derivatives. Gradients
// are closed over as typed arrays instead of saved as backend tensors because
// TFJS 4.22's WASM backend crashes when a custom op saves a freshly-created
// tensor; the mathematical derivative is unchanged.

function addPairGradient(gradient, left, right, valuesLeft, valuesRight, scale) {
  for (let axis = 0; axis < 3; axis += 1) {
    gradient[left + axis] += scale *
      (Number(valuesLeft[left + axis]) - Number(valuesRight[right + axis]));
  }
}

export function chamferL2CPU(tf, prediction, target) {
  const operation = tf.customGrad((predictionInput, targetInput) => {
    ...
    for (let batch = 0; batch < batches; batch += 1) {
      ...
      for (let predictionId = 0; predictionId < predictionCount; predictionId += 1) {
        ...
        for (let targetId = 0; targetId < targetCount; targetId += 1) {
          ...
          const predictionMap = batch * predictionCount + predictionId;
          if (distance < predictionMinimum[predictionMap]) {
            predictionMinimum[predictionMap] = distance;
            predictionNearest[predictionMap] = targetId;
          }
          const targetMap = batch * targetCount + targetId;
          if (distance < targetMinimum[targetMap]) {
            targetMinimum[targetMap] = distance;
            targetNearest[targetMap] = predictionId;
          }
        }
      }
    }
    ...
    for (let batch = 0; batch < batches; batch += 1) {
      ...
      const targetBase = batch * targetCount * 3;
      for (let predictionId = 0; predictionId < predictionCount; predictionId += 1) {
        const map = batch * predictionCount + predictionId;
        const predictionOffset = predictionBase + predictionId * 3;
        const targetOffset = targetBase + predictionNearest[map] * 3;
        predictionDistanceSum += predictionMinimum[map];
        addPairGradient(predictionGradient, predictionOffset, targetOffset,
          predictionValues, targetValues, predictionScale);
        addPairGradient(targetGradient, targetOffset, predictionOffset,
          targetValues, predictionValues, predictionScale);
      }
      for (let targetId = 0; targetId < targetCount; targetId += 1) {
        const map = batch * targetCount + targetId;
        const targetOffset = targetBase + targetId * 3;
        const predictionOffset = predictionBase + targetNearest[map] * 3;
        targetDistanceSum += targetMinimum[map];
        addPairGradient(targetGradient, targetOffset, predictionOffset,
          targetValues, predictionValues, targetScale);
        addPairGradient(predictionGradient, predictionOffset, targetOffset,
          predictionValues, targetValues, targetScale);
      }
    }
    return {
      value: tf.scalar(
      ...
      gradFunc: (outputGradient) => [
        tf.tensor(predictionGradient, predictionInput.shape, "float32").mul(outputGradient),
        tf.tensor(targetGradient, targetInput.shape, "float32").mul(outputGradient)
      ]
    };
  });
...
export function pointRepulsionLossCPU(tf, points, k = 8, radius = 0.04) {
  ...
  const operation = tf.customGrad((input) => {
    ...
    for (let batch = 0; batch < batches; batch += 1) {
      ...
      for (let left = 0; left < pointCount; left += 1) {
        ...
        for (let right = left + 1; right < pointCount; right += 1) {
          ...
          const distance = dx * dx + dy * dy + dz * dz;
          updateTopK(neighbourDistances, neighbourIds,
            mapBase + left * neighbours, neighbours, right, distance);
          updateTopK(neighbourDistances, neighbourIds,
            mapBase + right * neighbours, neighbours, left, distance);
        }
      }
    }
    ...
    return {
      ...
      gradFunc: (outputGradient) =>
        tf.tensor(gradient, input.shape, "float32").mul(outputGradient)
    };
  });
