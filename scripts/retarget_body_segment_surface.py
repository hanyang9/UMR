...
BODY_SEGMENT_SURFACE_COST_CONFIG_17 = {
    ...
    "leftArm": {"sample_slots": 15, "point_cost": 10.0, "normal_cost": 5.0},
    ...
    "rightArm": {"sample_slots": 15, "point_cost": 10.0, "normal_cost": 5.0},
    "leftHand": {"sample_slots": 15, "point_cost": 10.0, "normal_cost": 1.0},
    "rightLeg": {"sample_slots": 20, "point_cost": 10.0, "normal_cost": 5.0},
    "leftForeArm": {"sample_slots": 8, "point_cost": 10.0, "normal_cost": 5.0},
    "rightForeArm": {"sample_slots": 8, "point_cost": 10.0, "normal_cost": 5.0},
    "leftUpLeg": {"sample_slots": 25, "point_cost": 10.0, "normal_cost": 5.0},
    "hips": {"sample_slots": 40, "point_cost": 20.0, "normal_cost": 0.0},
    "leftShoulder": {"sample_slots": 30, "point_cost": 0.0, "normal_cost": 0.5},
    "rightShoulder": {"sample_slots": 30, "point_cost": 0.0, "normal_cost": 0.5},
}
BODY_SEGMENT_SURFACE_COST_CONFIG_19 = {
    **BODY_SEGMENT_SURFACE_COST_CONFIG_17,
    "leftUpperArm": {"sample_slots": 30, "point_cost": 0.0, "normal_cost": 0.1},
    "rightUpperArm": {"sample_slots": 30, "point_cost": 0.0, "normal_cost": 0.1},
}
...
def soma_slot_part_ids_from_nearest_joints(
    slot_points,
    source_template_joints,
    source_joint_names,
    log_prefix="Retarget",
):
    """Fallback SOMA body labels when the optional SMPLX-SOMA wrap assets are absent."""
    points = np.asarray(slot_points, dtype=np.float32)
    joints = np.asarray(source_template_joints, dtype=np.float32)
    names = [str(name) for name in (source_joint_names or [])]
    if points.ndim != 2 or points.shape[1] != 3:
        raise ValueError(f"SOMA slot points must be (N, 3), got {points.shape}")
    if joints.ndim != 2 or joints.shape[1] != 3 or len(joints) != len(names):
        raise ValueError(
            f"SOMA template joints/names mismatch: joints={joints.shape}, names={len(names)}"
        )

    def joint_part_name(name):
        lowered = str(name).lower()
        side = "left" if lowered.startswith("left") else "right" if lowered.startswith("right") else ""
        if side:
            if "hand" in lowered:
                return f"{side}Hand"
            if "forearm" in lowered:
            ...
            if "leg" in lowered:
                return f"{side}UpLeg"
        if any(token in lowered for token in ("head", "neck", "jaw", "eye")):
            return "head"
        if lowered in {"root", "hips", "pelvis"}:
            return "hips"
        return "torso"

    nearest_joint_ids = cKDTree(joints).query(points, k=1)[1]
    joint_part_ids = np.asarray(
        [BODY_SEGMENT_PART_IDS[joint_part_name(name)] for name in names],
        dtype=np.int32,
    )
    slot_part_ids = joint_part_ids[np.asarray(nearest_joint_ids, dtype=np.int32)]
    counts = {
        BODY_SEGMENT_PART_NAMES.get(int(part_id), str(int(part_id))): int(
            (slot_part_ids == part_id).sum()
        )
        for part_id in sorted(np.unique(slot_part_ids))
    }
    print(
        f"[{log_prefix}][WARN] SOMA body labels use nearest skeleton joints "
        f"because optional SMPLX-SOMA wrap assets are unavailable: {counts}"
    )
    return slot_part_ids.astype(np.int32)
...
def bind_source_slots_with_normals(
    ...
    if str(source_model_type).lower() == "soma":
        try:
            ...
            if source_joint_names is None or source_template_joints is None:
                raise
            slot_part_ids = soma_slot_part_ids_from_nearest_joints(
                binding["closest_points"],
                source_template_joints,
                source_joint_names,
                log_prefix=log_prefix,
            )
