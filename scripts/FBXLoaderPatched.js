	...
	// PI, so we'll interpolate large rotations
		interpolateRotations( curvex, curvey, curvez, eulerOrder ) {
			...
			function sampleCurve( curve, time ) {
				...
				if ( curve === undefined || curve.times.length === 0 ) return 0;
				const curveTimes = curve.times;
				const curveValues = curve.values;
				if ( time <= curveTimes[ 0 ] ) return curveValues[ 0 ];
				const last = curveTimes.length - 1;
				if ( time >= curveTimes[ last ] ) return curveValues[ last ];
				...
				while ( lo <= hi ) {
					...
					const mid = ( lo + hi ) >> 1;
					const midTime = curveTimes[ mid ];
					if ( midTime === time ) return curveValues[ mid ];
				...
				const right = lo;
				const left = right - 1;
				const span = curveTimes[ right ] - curveTimes[ left ];
				if ( span <= 0 ) return curveValues[ left ];
				const alpha = ( time - curveTimes[ left ] ) / span;
				return curveValues[ left ] + ( curveValues[ right ] - curveValues[ left ] ) * alpha;

			}

			times.forEach( function ( time ) {

				values.push( MathUtils.degToRad( sampleCurve( curves.x, time ) ) );
				values.push( MathUtils.degToRad( sampleCurve( curves.y, time ) ) );
				values.push( MathUtils.degToRad( sampleCurve( curves.z, time ) ) );
			...
			/*
			// Original three.js r165 code indexed Y/Z rotation curves by X-curve
			// frame index. That breaks FBX files with sparse per-axis rotation
			// keys, where X/Y/Z have different key counts and times. The patched
			// path above samples each axis at the union key times using linear
			// interpolation, avoiding stepped rotations at the start of clips.
